...
class TRS: 
    ...
    def get_trace_data(self, ind):  # Gives the data of the index_th trace
        data_ind = np.zeros((int(self.cryptolen)), np.dtype('B'))  #
        d_ind = self.traces[ind].data
        if ind < self.number_of_traces:
            for i in range(0, self.cryptolen):
                data_ind[i] = d_ind[i]
    ...
    def plot_trace(self, ind=0):
        y_data = self.get_trace_sample(ind) * 1000
...
if __name__ == "__main__":
    trs = TRS('t5.trs')  # The name of the trs file (name.trs)
    ...
    for i in range(trs.number_of_traces):
        ...
        trs.plot_trace(i)
    trs.plot_show('Samples', 'HW', 'Clock cycle', 'GILES')
